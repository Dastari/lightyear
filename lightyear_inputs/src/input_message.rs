...
pub trait ActionStateQueryData {
    ...
    // Bevy 0.19 split `IterQueryData` out of `QueryData`; mutable iteration (`iter_mut`) of a
    // query containing this data requires it. Concrete `Mut` types are mutable component refs,
    // which satisfy it.
