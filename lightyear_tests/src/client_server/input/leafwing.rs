...
fn test_input_message_with_spoofed_target_is_rejected() {
    ...
    let end_tick = stepper.server_tick() + 2;
    ...
    sequence_buf.set(end_tick, LeafwingSnapshot(snapshot_state));
    let sequence =
        LeafwingSequence::<LeafwingInput1>::build_from_input_buffer(&sequence_buf, 1, end_tick)
    ...
    forged.inputs.push(PerTargetData {
        target: InputTarget::Entity(victim_local_on_client_0),
        states: sequence.clone(),
    });
    forged.inputs.push(PerTargetData {
        target: InputTarget::Entity(attacker_local_on_client_0),
        states: sequence,
    });
    ...
    let attacker_server_buffer = stepper
        .server_app
        .world()
        .entity(attacker_server_entity)
        .get::<LeafwingBuffer<LeafwingInput1>>()
        .expect("attacker has InputBuffer");
    assert!(
        attacker_server_buffer
            .get_last()
            .is_some_and(|snapshot| snapshot.0.pressed(&LeafwingInput1::Jump)),
        "Client 0's legitimate Jump input for its OWN entity did not \
         reach the server InputBuffer. \
